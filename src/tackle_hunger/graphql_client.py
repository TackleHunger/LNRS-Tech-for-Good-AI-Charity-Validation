...
class TackleHungerConfig:
    ...
    def __init__(self, 
        ...
        self.ai_scraping_token = ai_scraping_token or os.getenv("AI_SCRAPING_TOKEN", "")
        self.environment = environment or os.getenv("ENVIRONMENT", "dev")

        if not self.ai_scraping_token and self.environment != "dev":
            raise ValueError(
                "AI_SCRAPING_TOKEN environment variable is required for "
                f"{self.environment} environment. Set it in .env or export it."
            )
